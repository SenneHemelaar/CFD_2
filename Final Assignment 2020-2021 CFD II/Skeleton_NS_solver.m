...
% filename to save workspace to for post-processing
...
u_vec = zeros(4*N,1);
for i = 1:N
    u_vec(1+2*(i-1)) = U_wall_left;
    u_vec(2*i)       = U_wall_top;
    u_vec(2*N+i)     = V_wall_bot;
    u_vec(3*N+i)     = V_wall_top;
end
[tE21, u_norm] = Seperate_tE21(tE21, N, u_vec);
